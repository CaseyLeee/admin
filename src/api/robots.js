...
export function queryRobotModel(data) {
    return request({
        method: "get",
        url: "/rosWeb/robotModel/queryRobotModel",
        data
    })
}
/**
 ...
 *  deviceSn	String	设备sn号
    password	String	设备密码（默认6666）
 * @param {*} params 
 * 
 */
export function resetRobotPsw(data) {
    return request({
        method: "post",
        url: "/app/2.1.0/device/reset/password",
        data
    })
}
/**
 ...
 * 
 */
export function handleModifyInBatch(data, token='xxx') {
   
    return request({
        method: "post",
        url: "/app/2.1.0/device/batch/reset/OpenDoorpassword",
        data,
        headers: { "Content-Type": " application/json", msToken: token }
    })
}